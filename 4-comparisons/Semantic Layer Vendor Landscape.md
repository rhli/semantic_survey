...
| 公司 | 产品 | 开源 | 要点 |
|---|---|---|---|
| dbt Labs | [[dbt Semantic Layer]]（MetricFlow） | MetricFlow 开源（Apache 2.0），serving 需 dbt Cloud | metrics-as-code，YAML 与 dbt 模型同仓；MetricFlow 是 OSI 规范基础 |
...
- **[[Open Semantic Interchange]]（OSI）**：2026-01 发布的厂商中立语义定义标准（YAML，基于 MetricFlow 规范），Snowflake、dbt、Databricks、Salesforce 等 40+ 厂商参与，Apache 孵化（"Ossie"）。语义碎片化问题的行业级回应。
...
- **Tier 1 深研**（产品页 + 技术机制）：Databricks、Snowflake、dbt Labs、Cube、AtScale、Atlan、Aloudata
